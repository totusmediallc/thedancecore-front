...
const UserFormModal = ({ mode, visible, submitting, onClose, onSubmit, user, disableRole }) => {
  ...
  const validate = useCallback(() => {
    ...
    if (!['admin', 'client'].includes(formState.role)) {
          ...
          <CRow className="g-3">
            ...
            <CCol md={6}>
              ...
              <CFormSelect
                ...
                disabled={submitting || disableRole}
...
const DeleteConfirmModal = ({ visible, onClose, onConfirm, user, submitting }) => (
  ...
  >
    ...
    <CModalBody>
      <p className="mb-3">
        ¿Estás seguro de que deseas desactivar a <strong>{user?.firstName}</strong>?
      </p>
...
const UsersManagement = () => {
  ...
  const normalizedFilters = useMemo(() => {
    return {
      ...
      isActive: filters.isActive === '' ? undefined : filters.isActive === 'true' ? true : false,
    }
  }, [filters])
  ...
  }, [filters.limit, filters.page, isAdmin, normalizedFilters])
  ...
  return (
    <>
      <CRow className="g-4">
        <CCol xs={12}>
          <CCard className="shadow-sm border-0">
            <CCardHeader className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
              <div>
                <h2 className="h5 mb-1">Administración de usuarios</h2>
                <p className="text-body-secondary mb-0">
                  Gestiona las cuentas del sistema: crea, edita y desactiva usuarios.
                </p>
              </div>
                      ...
                      users.map((user) => {
                        ...
                        return (
                          <CTableRow key={user.id} className="align-middle">
                            ...
                            <CTableDataCell>
                              {user.role === 'admin' ? (
                                <CBadge color="dark" className="fw-semibold">
                                  Admin
                                </CBadge>
                              ) : (
                                <CBadge color="info" className="text-dark fw-semibold">
                                  Cliente
                                </CBadge>
                              )}
                            </CTableDataCell>
                            ...
                            <CTableDataCell className="text-end">
                              <CButtonGroup role="group" className="justify-content-end">
                                ...
                                >
                                  <CIcon icon={cilTrash} className="me-1" /> Desactivar
                                </CButton>
                              </CButtonGroup>
                            </CTableDataCell>
                          </CTableRow>
      ...
      <UserFormModal
        ...
        disableRole={Boolean(
          editModalState.user?.role === 'admin' && currentUser?.id !== editModalState.user?.id,
        )}
      />
      <DeleteConfirmModal
