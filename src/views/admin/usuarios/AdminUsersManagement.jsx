...
const UserFormModal = ({ mode, visible, submitting, onClose, onSubmit, user, academies = [] }) => {
  ...
  const handleChange = (field) => (event) => {
    ...
    setFormState((prev) => {
      ...
      // Limpiar academyId si el rol no requiere academia
...
const DeleteConfirmModal = ({ visible, onClose, onConfirm, user, submitting }) => (
  ...
  >
    ...
    <CModalBody>
      <p className="mb-3">
        ¿Estás seguro de que deseas desactivar a <strong>{user?.firstName} {user?.lastName}</strong>?
      </p>
...
const AdminUsersManagement = () => {
  ...
  const canManagePermissions = hasPermission(PERMISSIONS.USERS_MANAGE_PERMISSIONS)
  ...
  const loadAcademies = useCallback(async () => {
    try {
      ...
      setAcademies(Array.isArray(response?.data) ? response.data : Array.isArray(response) ? response : [])
    } catch (requestError) {
  ...
  const canEditUser = useCallback(
    (user) => {
      ...
      // No puede editar otros admin si no es el mismo
          ...
          <CCard className="shadow-sm border-0">
            <CCardHeader className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
              <div>
                ...
                <p className="text-body-secondary mb-0">
                  Gestiona las cuentas del sistema: crea, edita y administra permisos de usuarios.
                </p>
              </div>
